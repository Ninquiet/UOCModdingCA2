...
namespace Sisus
{
	...
	public sealed class DefaultDrawerProvider : DrawerProviderBase
	{
		...
		private void SetupOnBackgroundThread()
		{
		...
		protected override void RebuildDrawers()
		{
			isReady = false;
			setupData = null;
		...
		private IEnumerable<Task> GetAllSetupTasks(IEnumerable<Type> drawerTypes)
		{
			...
			var unityObjectTypes = TypeCache.GetTypesDerivedFrom<Object>();

			yield return Task.WhenAll(unityObjectTypes.Select(BuildDictionariesForCustomEditorsInNonUnityNamespacesTask));
			...
			yield return Task.WhenAll(unityObjectTypes.Select(BuildDictionariesForCustomEditorsInUnityNamespacesTask));
		}
		...
		private Task BuildDictionariesForCustomEditorsInNonUnityNamespacesTask(Type unityObjectType)
		{
			return Task.Run(() => BuildDictionariesForCustomEditorsInNonUnityNamespaces(unityObjectType));
		}
		private void BuildDictionariesForCustomEditorsInNonUnityNamespaces(Type unityObjectType)
		{
			if(CustomEditorUtility.TryGetCustomEditorType(unityObjectType, out Type editorType, true) && !IsInUnityNamespace(editorType))
			{
		...
		private Task BuildDictionariesForCustomEditorsInUnityNamespacesTask(Type unityObjectType)
		{
			return Task.Run(() => BuildDictionariesForCustomEditorsInUnityNamespaces(unityObjectType));
		}
		private void BuildDictionariesForCustomEditorsInUnityNamespaces(Type unityObjectType)
		{
			if(CustomEditorUtility.TryGetCustomEditorType(unityObjectType, out Type editorType, true) && IsInUnityNamespace(editorType))
			{
