...
namespace Sisus
{
	...
	public abstract class DrawerProviderBase : IDrawerProvider
	{
		...
		public event Action<IDrawerProvider> OnBecameReady
		{
			add
			{
				if(IsReady)
		...
		protected void BroadcastOnBecameReady() => onBecameReady?.Invoke(this);
		...
		protected abstract void RebuildDrawers();
		...
		private Type GetDrawerTypeByName([NotNull]string drawerName, Type interfaceType)
		{
			...
			if(!DrawersByInterfaceTypeThenName.TryGetValue(interfaceType, out drawersByName))
			{
				...
				foreach(var drawerType in TypeExtensions.GetImplementingTypes(interfaceType, false))
				{
					...
					// Type.FullName can be null for open generic types like Sisus.FieldDrawer<T>
					if(drawerType.FullName is string fullName)
					{
						drawersByName[fullName] = drawerType;
					}
					#if DEV_MODE && DEBUG_GET_DRAWER_BY_NAME
					else { Debug.Log($"GetDrawerTypeByName(\"{drawerName}\", {interfaceType.Name}) - Drawer {TypeExtensions.GetFullName(drawerType)} FullName was null."); }
					#endif
				}
