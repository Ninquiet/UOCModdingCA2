...
namespace Sisus
{
	...
	public static class TypeExtensions
	{
		...
		internal static string GetPopupMenuLabel([NotNull]Type type, [NotNull]string globalNamespaceTypePrefix)
		{
		...
		internal static void GetPopupMenuLabel([NotNull]Type type, [NotNull]string globalNamespaceTypePrefix, StringBuilder sb)
		{
			if(type.Namespace == null)
			{
				sb.Append(globalNamespaceTypePrefix);
			}

			GetPopupMenuLabel(type, sb);
		}

		internal static string GetPopupMenuLabel([NotNull]Type type)
		{
			#if DEV_MODE
			Debug.Assert(IsReady);
			#endif

			if(TypePopupMenuPaths.TryGetValue(type, out string result))
			{
				return result;
			}
		...
		internal static void GetPopupMenuLabel([NotNull]Type type, StringBuilder sb)
		{
			if(TypePopupMenuPaths.TryGetValue(type, out string cachedResult))
			{
		...
		public static string GetShortName([NotNull]Type type)
		{
			...
			var sb = new StringBuilder();
			StringUtils.ToString(type, '/', sb);
			TypePopupMenuPaths[type] = sb.ToString();
