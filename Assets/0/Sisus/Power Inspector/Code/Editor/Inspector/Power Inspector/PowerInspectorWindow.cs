...
namespace Sisus
{
	...
	public sealed class PowerInspectorWindow : InspectorDrawerWindow<PowerInspectorWindow, PowerInspector>, IHasCustomMenu
	{
		...
		public void AddItemsToMenu(GenericMenu menu)
		{
			...
			menu.AddItem(new GUIContent("Refresh\tF5"), false, PerformFullRefresh);
			menu.AddItem(new GUIContent("Full Refresh\tShift+F5"), false, PerformFullRefresh);
		...
		public void PerformFullRefresh()
		{
			...
			bool viewIsLocked = MainView.State.ViewIsLocked;
			var inspected = MainView.State.inspected.Clone() as Object[];
			...
			{
				if(focusedWindow is PowerInspectorWindow newWindow
					&& newWindow != null
					&& newWindow.MainView is IInspector newInspector)
				{
					newInspector.State.ViewIsLocked = viewIsLocked;
					newInspector.RebuildDrawers(inspected, false);
				}
			};
		}
